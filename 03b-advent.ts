...
type Pos = [number, number] & { steps: number };
...
function unitVec(dir: CardinalDir): [number, number] {
  if (dir === 'U') return [0, 1];
  if (dir === 'D') return [0, -1];
  if (dir === 'L') return [-1, 0];
  if (dir === 'R') return [1, 0];
...
function dist(a: Pos, c: Pos): number {
...
function segmentsIntersect(ab: Segment, cd: Segment): Pos | null {
  ...
  if (abVerticalCdVertical === 'tt') {
    ...
    const yInter = intervalsIntersect(
      toInterval([a[1], b[1]]),
      toInterval([c[1], d[1]]));
    if (!yInter) return null;
    const pos = [a[0], yInter] as Pos;
    pos.steps = a.steps + c.steps + dist(a, c);
    return pos;
    ...
    const xInter = intervalsIntersect(
      toInterval([a[0], b[0]]),
      toInterval([c[0], d[0]]));
    if (!xInter) return null;
    const pos = [xInter, a[1]] as Pos;
    pos.steps = a.steps + c.steps + dist(a, c);
    return pos;
